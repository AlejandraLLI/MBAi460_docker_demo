...
def lambda_handler(event, context):
    ...
    artifacts = Path(config.get("run_config",{}).get("output", "/results"))
    # Debug log to check if the directory already exists
    if artifacts.exists():
        logger.info("Artifacts directory already exists: %s", artifacts)
    else:
        logger.info("Artifacts directory does not exist, creating: %s", artifacts)
